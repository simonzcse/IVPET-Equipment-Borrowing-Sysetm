 ...
 * @author simon
 */
@WebServlet(name = "handleEquipmentManangement", urlPatterns = {"/Tech/handleEquipmentManangement"})
public class handleEquipmentManangement extends HttpServlet {
private TechDB2 db;
...
public void init() {
     ...
     protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            if ("List".equalsIgnoreCase(action)) {
               ...
               rd = getServletContext().getRequestDispatcher("/Tech/Tech_List_All_Equipment.jsp");
               rd.forward(request, response);
                ...
                while (rs.next()){
                    ...
                    RequestDispatcher rd;
                    rd = getServletContext().getRequestDispatcher("/Tech/Tech_Edit_Equipment.jsp");
                    rd.forward(request, response);
                }
            }
                ...
                if(isBorow.equals("0")){
                    int isEdit = db.editBySQL("UPDATE Equipment_Each SET Equipment_Each_isAvailable=false WHERE Equipment_Each_ID="+equipment_Each_ID+";");
                if (isEdit!=0){
                ...
                int equipment_Each_ID = Integer.parseInt(request.getParameter("Equipment_Each_ID"));
                    int isEdit = db.editBySQL("UPDATE Equipment_Each SET Equipment_Each_isAvailable=true WHERE Equipment_Each_ID="+equipment_Each_ID+";");
                if (isEdit!=0){
                ...
                int equipment_Each_ID = Integer.parseInt(request.getParameter("Equipment_Each_ID"));
                    int isEdit = db.editBySQL("Delete From Equipment_Each WHERE Equipment_Each_ID="+equipment_Each_ID+";");
                if (isEdit!=0){
                ...
                while (rs.next()){
                    ...
                    RequestDispatcher rd;
                    rd = getServletContext().getRequestDispatcher("/Tech/Tech_Add_Equipment.jsp");
                    rd.forward(request, response);
     }  
                ...
                int isEdit = db.editBySQL(s);
                if (isEdit!=0){
                    int isEdit2 = db.editBySQL(s2);
                ...
                for (int i=0;i<count;i++){
                    db.editBySQL("insert into Equipment_Each (Equipment_ID, Equipment_Each_isBorow, Equipment_Each_isAvailable) VALUES("+equipment_ID+", false, true);");
                }
                ...
                int isEdit = db.editBySQL("UPDATE Equipment SET Equipment_Name='"+equipment_Name+"', Equipment_Type='"+equipment_Type+"' WHERE Equipment_ID="+equipment_ID+";");
                if (isEdit!=0){
                    out.println("<script>alert('Successfully')</script>");
                    response.sendRedirect("handleEquipmentManangement?Action=List");
                    }
