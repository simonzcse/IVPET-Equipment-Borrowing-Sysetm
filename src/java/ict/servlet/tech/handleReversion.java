 ...
 * @author simon
 */
@WebServlet(name = "handleReversion", urlPatterns = {"/Tech/handleReversion"})
public class handleReversion extends HttpServlet {
private TechDB2 db;
    /**
...
public void init() {
     ...
     protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            if ("Reject".equalsIgnoreCase(action)) {
                ...
                int reversion_ID = Integer.parseInt(request.getParameter("Reversion_ID"));
                int isEdit = db.editBySQL("UPDATE reversion SET Reversion_Status='Approve' WHERE Reversion_ID="+reversion_ID+";");
                if (isEdit!=0){
                ...
                int isEdit = db.editBySQL("UPDATE Borrow_Equipment SET Borrow_Equipment_Status='Returned' WHERE Borrow_ID="+borrow_ID+" and Equipment_Each_ID="+equipment_Each_ID+";");
                if (isEdit!=0){
                    int isEdit2 = db.editBySQL("UPDATE Equipment_Each SET Equipment_Each_isBorow=false WHERE Equipment_Each_ID="+equipment_Each_ID+";");
                ...
                rd = getServletContext().getRequestDispatcher("/Tech/Tech_Borrowing_Request.jsp");
                out.println("Database No Change");
                ...
                request.setAttribute("reversionBeans", reversionBeans);
                RequestDispatcher rd;
                rd = getServletContext().getRequestDispatcher("/Tech/Tech_Borrow.jsp");
                out.println("Database No Change");
                rd.forward(request, response);
            }
                ...
                RequestDispatcher rd;
                rd = getServletContext().getRequestDispatcher("/Tech/Tech_BorrowSelectEach.jsp");
                rd.forward(request, response);
                ...
                if(borrowBeans!= null){
                    ...
                    db.editBySQL("insert into Borrow(Account_ID, Reversion_ID, Borrow_StartDate, Borrow_ExpirationDate) values("+account_ID+", "+RreversionID+",'"+today+"', '"+expirationDate+"');");
                    db.editBySQL("UPDATE Reversion SET Reversion_Status='Borrowed' WHERE Reversion_ID="+RreversionID+";");
                    ...
                    for (int i = 0; i<borrowBeanList.size();i++) {
                        ...
                        int isupdate=db.editBySQL("insert into Borrow_Equipment values("+maxBorrow_ID+", "+b.getEquipmentEachID()+", 'Borrowing');");
                        out.println(b.getEquipmentEachID());
                        if(isupdate>0){
                        db.editBySQL("UPDATE Equipment_Each SET Equipment_Each_isBorow=1 WHERE Equipment_Each_ID="+b.getEquipmentEachID()+";");}
                    }
               ...
               RequestDispatcher rd;
               rd = getServletContext().getRequestDispatcher("/Tech/Tech_Return.jsp");
               out.println("Database No Change");
               rd.forward(request, response);
