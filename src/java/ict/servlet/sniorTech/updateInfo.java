    ...
    protected void processRequest(HttpServletRequest req, HttpServletResponse res)
        ...
        try (PrintWriter out = res.getWriter()) {
            ...
            new TechDB("jdbc:mysql://localhost:3306/itp4511_project?useSSL=false", "root", "")
                    .updateSQL("UPDATE `account` SET `Account_Name` = '"+ studentName +"',Account_Email = '"+ studentEmail +"', Account_Password = '"+studentPassword+"', Account_Role = '"+studentRole+"' WHERE `account`.`Account_ID` = "+studentID);
            
            out.println("<script>alert('Successfully edited!');</script>");
        }
    }
